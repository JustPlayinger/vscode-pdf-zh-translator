...
(function () {
  ...
  /**
   * 显式把 pdf.js 的 worker 地址设成**绝对 URL**。
   *
   * 正常情况下 worker 由 <script src="...pdf.worker.js"> 作为普通脚本载入，
   * pdf.js 会直接复用 globalThis.pdfjsWorker，不再加载任何脚本。
   * 但万一那段脚本没跑起来，pdf.js 会退化为「假 worker」并加载
   * GlobalWorkerOptions.workerSrc —— 而 viewer.js 的默认值是**相对路径**
   * "../build/pdf.worker.js"，在 Webview 里会被解析成
   * vscode-webview://<id>/build/pdf.worker.js 从而 404。
   * 这里给出绝对地址，彻底消除该风险。
   */
  function applyWorkerSrc(config) {
    ...
    try {
      ...
      console.warn('[pdf-zh] 设置 GlobalWorkerOptions.workerSrc 失败：' + messageOf(error));
    }
    try {
      ...
      console.warn('[pdf-zh] 设置 pdf.js 选项 workerSrc 失败：' + messageOf(error));
    }
  }
  ...
  window.__PDF_ZH__ = {
    ...
    applyWorkerSrc: applyWorkerSrc,
    showNotice: showNotice,
  };

  // 尽早把绝对 worker 地址交给 pdf.js，避免它退化时才用上相对默认值。
  applyWorkerSrc(initialConfig);
  ...
  window.addEventListener(
    ...
    async function () {
      ...
      // worker 脚本若没以 <script> 方式跑起来，pdf.js 会走「假 worker」分支，
      // 这里把线索留到控制台，便于一眼定位。
      applyWorkerSrc(config);
      if (typeof globalThis !== 'undefined' && !globalThis.pdfjsWorker) {
        console.warn(
          '[pdf-zh] 未检测到 globalThis.pdfjsWorker：worker 脚本可能未被加载，' +
            '将依赖 workerSrc=' +
            config.workerSrc +
            ' 的假 worker 回退。',
        );
