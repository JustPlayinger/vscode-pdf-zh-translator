...
(function () {
  const bridge = window.__PDF_ZH__ || {};
  const vscode = bridge.vscode;
  ...
  function ensureUi() {
    ...
    panel.addEventListener('click', function (event) {
      ...
      if (act === 'page') {
        ...
        vscode.postMessage({ type: 'tr:export', format: 'dual' });
      } else if (act === 'stats') {
        vscode.postMessage({ type: 'tr:stats' });
      } else if (act === 'close') {
        setPanel(false);
      }
    });
  ...
  function request(payload) {
    return new Promise(function (resolve, reject) {
      ...
      vscode.postMessage(Object.assign({ type: 'tr:request', reqId: reqId }, payload));
    });
  }
  ...
  function stopTranslation() {
    state.aborted = true;
    vscode.postMessage({ type: 'tr:abort' });
  ...
  function start() {
    ...
    setBusy(false);
    wireViewer();
    vscode.postMessage({ type: 'tr:ready' });
  }
